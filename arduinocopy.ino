...
#define Br 1      // This is where you define the brightness of the LEDs - this is constant for all
...
Adafruit_NeoPixel meridians[25];    // create meridians object array for 24 meridi0a00    ns + one daisy-chained central strip
...
unsigned long previousMillis, currentMillis, sweep_interval = 100000;  // the interval for the sweep in kinetic perimetry (in ms)
byte sweepStart, longitudeInt, Slider = 255, currentSweepLED, sweepStrip, daisyStrip;
byte meridians_turnOn[24];
byte fixationStrength = 100; //brightness of the fixation
...
/*************************************************************************************************/
  ...
  for(int pixelNumber = 0; pixelNumber < numPixels[24]; pixelNumber++) {
    uint32_t color = meridians[24].getPixelColor(pixelNumber);
    Serial.println("Daisy " + String(pixelNumber) + " " + String(color));
  }*/
  ...
  if (sweep == true) {
    ...
    if(currentMillis - previousMillis <= sweep_interval) {
           ...
           if (currentSweepLED > 3) {
             ...
             sweep_interval = 5000;      // infinitely short so that we just zap off the longer strip
             ...
             sweep_interval = 5000;    // this figure should be properly calibrated
           }
